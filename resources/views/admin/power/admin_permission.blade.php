...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l"><a href="javascript:;" onclick="datadel()"
                ...
                href="javascript:;" onclick="admin_permission_add('添加权限节点','{{url('admin/node/create')}}','','310')"
                class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加权限节点</a></span> <span class="r">共有数据：<strong>{{count($power)}}</strong> 条</span>
    </div>
    <table class="table table-border table-bordered table-bg">
        ...
        <tr class="text-c">
            ...
            <th width="200">路由别名</th>
            <th width="200">是否菜单</th>
        ...
        @foreach($power as $item)
            <tr class="">
                ...
                <td>{{$item['id']}}</td>
                <td>{{str_repeat('——',$item['level'])}}{{$item['node_name']}}</td>
                <td>{{$item['route']}}</td>
                <td>{{$item['is_menu']}}</td>
                <td>{{$item['created_at']}}</td>
                <td>
                    <a title="编辑" href="javascript:;"
                       onclick="admin_permission_edit('角色编辑','{{url("admin/node/".$item['id']."/edit")}}',{{$item['id']}},'','310')"
                       class="ml-5"
                       style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除"
                                                                                                href="javascript:;"
                                                                                                onclick="admin_permission_del(this,{{$item['id']}})"
                                                                                                class="ml-5"
...
<script type="text/javascript">
    ...
    */
    ...
    function admin_permission_edit(title, url, id, w, h) {
        $.ajax({
            url: 'http://www.hkzf.com/admin/checkNode',
            data: {id: id},
    ...
    function admin_permission_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            $.ajax({
                url: 'http://www.hkzf.com/admin/node/'+id,
                method: "DELETE",
                headers: {
