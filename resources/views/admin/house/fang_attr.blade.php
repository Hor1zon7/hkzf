...
<div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l"><a href="javascript:;" onclick="datadel()"
            ...
            <a href="javascript:;" onclick="house_add('添加房源属性','{{url('admin/fangattr/create')}}','800','500')"
               class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>添加房源属性</a>
...
<table class="table table-border table-bordered table-bg" id="datatables">
    <thead>
    <tr>
        <th scope="col" colspan="9">房源属性列表</th>
    </tr>
    <tr class="text-c">
        <th width="25"><input type="checkbox" name="" value=""></th>
        <th width="40">ID</th>
        <th width="50">房源属性名称</th>
        <th width="90">图标</th>
        <th width="40">字段名称</th>
        <th width="40">加入时间</th>
        <th width="100">操作</th>
    ...
    @foreach($data as $item)
        <tr class="text-c">
            ...
            <td style="text-align: left">{{str_repeat('——',$item['level']).$item['name']}}</td>
            <td><img src="{{$item['icon']}}" alt="" width="50px"></td>
            <td>{{$item['filed_name']}}</td>
            ...
            <td class="td-manage"><a style="text-decoration:none" onClick="admin_stop(this,'10001')"
                    ...
                    onclick="admin_edit('房源属性编辑','{{url("admin/fangattr/".$item['id']."/edit")}}','1','800','500')"
...
<script type="text/javascript">
    ...
    */
    ...
    function excel() {
        $.ajax({
            url: "http://www.hkzf.com/admin/excel",
            data: {
                type: 'fangattr'
            },
    ...
    function admin_del(obj, id) {
        ...
        layer.confirm('确认要删除吗？', function (index) {
            $.ajax({
                type: 'DELETE',
                url: 'http://www.hkzf.com/admin/fangattr/' + id,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                },
                data: {
