...
@section('content')
    <article class="page-container">
        <form class="form form-horizontal" id="form-article-add" action="{{url('admin/fangattr/'.$data['id'])}}" >
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>是否顶级：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <select name="pid" id="">
                        <option value="0">==顶级==</option>
                        @foreach($list as $v)
                            @if($v['level']==0)
                                <option value="{{$v['id']}}" @if($data['pid']==$v['id']) selected @endif >{{str_repeat('——',$v['level']).$v['name']}}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
...
{{--            @method('PUT')--}}
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" value="{{$data['filed_name']}}" name="filed_name">
                </div>
            </div>
            ...
            <div class="row cl">
                ...
                <div class="imgs">
                    ...
                    <img src="{{$data['icon']}}" alt="" id="image" style="width: 100px;height: 100px">
                </div>
                <div class="formControls col-xs-8 col-sm-9">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                $('#image').css('display', '');
                $('#image').attr('src', src);
                $('input[name=icon]').attr('value', src);
            })
